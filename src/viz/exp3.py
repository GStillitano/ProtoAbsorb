...
def _add_vertical_block_bands(ax, L: int, y_label_pos: float = 0.97) -> None:
    """Vertical colour bands + block labels for a plot with layer on x-axis."""
    for lo, hi, label, color in _BLOCK_SPANS:
        if lo >= L:
            break
        hi = min(hi, L - 1)
        ax.axvspan(lo - 0.5, hi + 0.5, color=color, alpha=0.25, zorder=0)
        ax.text(
            (lo + hi) / 2, y_label_pos, label.split("\n")[0],
...
def _input_embedding_arrows(ax) -> None:
    """Add '← input' and 'embedding →' labels to x-axis."""
    ax.annotate(
    ...
    ax.annotate(
        ...
        ha="right", va="top", fontsize=8, color="salmon",
        fontweight="bold",
    )
...
def plot(results: dict, out_dir: Path) -> None:
    ...
    for data, ax, title in [
        (gamma_drift, axes[0], "‖Δγ‖₂  (gamma drift)"),
        (beta_drift,  axes[1], "‖Δβ‖₂  (beta drift)"),
        (cv_gamma,    axes[2], "CV(Δγ)  (structured vs uniform drift)"),
    ]:
        # data shape (T+1, L): rows = time, cols = layer → imshow maps rows→y, cols→x
        im = ax.imshow(
            data, aspect="auto", origin="upper",
            extent=[-0.5, L - 0.5, T + 0.5, -0.5],
        )
        _add_vertical_block_bands(ax, L)
    ...
    for ax, data, ylabel in [
        ...
        ax.bar(xticks, data, color=bar_colors, edgecolor="none", width=0.8)
        _add_vertical_block_bands(ax, L)
    ...
    patches = [mpatches.Patch(color=c, label=lbl.split("\n")[0])
               for _, _, lbl, c in _BLOCK_SPANS if _ < L]
